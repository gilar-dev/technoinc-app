...
export function ContributionHeader({ title = "" }: ContributionHeaderProps) {
    return (
        <div className="mx-3 max-w-5xl py-3 lg:mx-14 lg:py-5 xl:mx-21">
            <div className="rounded-lg border border-sidebar-border bg-form-bg p-5 shadow-sm shadow-black/10 lg:p-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        ...
                        <h1 className="mt-1 font-montserrat text-2xl font-bold">{title ? "Edit article" : "Create article"}</h1>
                        <p className="mt-1 text-sm text-foreground/65">{title ? "You have become a contributor by editing this article." : "Create new article and become a contributor."}</p>
                    </div>
                    {title && (
